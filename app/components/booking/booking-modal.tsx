...
const BookingModalClient = dynamic(() => import("./booking-modal-client"), { ssr: false });
...
export default function BookingModal({ triggerText = "Book My Free SEO Strategy Call" }: { triggerText?: string }) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center bg-navy-500 bg-opacity-70 p-4 md:p-6 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg relative max-h-[calc(100dvh-2rem)] md:max-h-[calc(100dvh-3rem)] overflow-hidden">
            <button
