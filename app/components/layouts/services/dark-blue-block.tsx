...
export default function DarkBlueBlock({ icon: Icon, iconRoute, iconDescription, iconWidth, title, imageSrc, blockId, onClick, top }: BlockProps) {
    ...
    (<div className="group relative flex flex-col w-full aspect-square items-center justify-center gap-[10px] cursor-pointer" onClick={onClick}>
      ...
      >
        {Icon && (
          <Icon
            aria-hidden="true"
            className={`absolute left-1/2 transform -translate-x-1/2 ${top} w-16 h-16 text-white`}
          />
        )}
        <h4
